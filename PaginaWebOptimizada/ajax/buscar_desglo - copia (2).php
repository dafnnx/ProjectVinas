		...
		if ($numrows>0){
			...
			<div class="">
			  ...
			  <table class="table" data-responsive="table" id="resultTable">
				<?php
				$query=$db2->prepare("SELECT * FROM tratamientos WHERE id_residente=:idr");
				...
				for($i=0; $row = $quey->fetch(); $i++){	 
					...
					echo $nmed;  } ?>					
						...
						<td class="textcenter w30 bgmain padd5 cpoint" onclick="mdlapp(modapp, 'appclose', '<?php echo $idt; ?>', '8h')"><?php echo $och; ?></td>
						<td class="textcenter w30 bgmain padd5 cpoint" onclick="mdlapp(modapp, 'appclose', '<?php echo $idt; ?>', '13h')"><?php echo $trce; ?></td>
						<td class="textcenter w30 bgmain padd5 cpoint" onclick="mdlapp(modapp, 'appclose', '<?php echo $idt; ?>', '18h')"><?php echo $dieco; ?></td>
						<td class="textcenter w30 bgmain padd5 cpoint" onclick="mdlapp(modapp, 'appclose', '<?php echo $idt; ?>', '21h')"><?php echo $vtuno; ?></td>
						<td><?php 
...
AND (:fec BETWEEN (SELECT count(*) AS numrows FROM tratamientos WHERE id_residente=:idr) AND :ffin)