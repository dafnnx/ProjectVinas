...
<?php
 $rid= $_POST['rid'];
    ...
    $count_query= $db2->prepare("SELECT count(*) AS numrows FROM hist_inventarios WHERE id_residente=:id");
    $count_query->bindParam(':id', $rid);
    $count_query->execute();
    for($i=0; $row = $count_query->fetch(); $i++){
    $numrows = $row['numrows'];}
    $query=$db2->prepare("SELECT * FROM hist_inventarios WHERE id_residente=:id ORDER BY fec_notamed DESC");
