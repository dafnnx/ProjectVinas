...
$fec=$_POST['fec'];
	require_once ("../cn/connect2.php"); 	
		...
		$numrows = $row['numrows'];}
/*
		$quy=$db2->prepare("SELECT fecha_ini AS fini, fecha_fin AS ffin FROM tratamientos WHERE id_residente=:idr AND status_tratamiento=1");
		$quy->bindParam(':fec', $fec);
		$quy->bindParam(':idr', $idr);
		$quy->execute();
				for($i=0; $row = $quy->fetch(); $i++){
					$fini=$row['fini'];
					$ffin=$row['ffin'];
*/
		$query=$db2->prepare("SELECT * FROM tratamientos WHERE id_residente=:idr AND status_tratamiento=1/* AND (:fec BETWEEN :fini AND :ffin) */");
		$query->bindParam(':fini', $fini);
		$query->bindParam(':ffin', $ffin);
