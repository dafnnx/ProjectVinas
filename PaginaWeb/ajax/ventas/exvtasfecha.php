...
$activeSheet->setCellValue('E1', 'TOTAL');
	...
	$activeSheet->setCellValue('E'.$i , $debiv);	
}

$amount = $db2->prepare("SELECT sum(debe_pay) AS am, SUM(iva_pay) AS iv FROM payconcept WHERE (fecha_pay BETWEEN :fin AND :ffi)  AND id_residente=$idr AND status=2");
...
if ($iv) {	$div= $stot+$iv;	}
   			...
   			$activeSheet->setCellValue('F'.$i , $div);}
