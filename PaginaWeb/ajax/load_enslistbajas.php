...
if($action == 'ajax'){
    ...
    if ($numrows > 0) { ?>
        <div class="">
            ...
            <script>
                // Establecer el nombre del residente para uso en JavaScript
                if (typeof setResidentName === 'function') {
                    setResidentName('<?php echo addslashes($resident_name); ?>');
                } else {
                    // Si la función no existe aún, almacenar temporalmente
                    window.tempResidentName = '<?php echo addslashes($resident_name); ?>';
                }
            </script>
...
// Función para obtener el nombre del residente (llamar al inicio de la página)
...
function showReactivateModal() {
    ...
    // NO usar aria-hidden, solo mostrar el modal
...
function reactivateSelectedEnseres() {
    ...
    $('#reactivate_rid').val($('#tgtrid').val()); // Obtener rid del input hidden
    
    // Mostrar modal con información detallada
    showReactivateModalDetailed(selectedIds, currentResidentName);
}
...
function reactivateSingleEnsere(id, rid) {
    ...
    showReactivateModalDetailed([id], currentResidentName);
}
...
function showReactivateModalDetailed(selectedIds, residentName) {
    ...
    $('#reactivate-resident-name').text(residentName || 'No disponible');
...
// Función CORREGIDA para procesar la reactivación
        ...
        success: function(response) {
            ...
            if (response.includes('success')) {
                Swal.fire({
                    ...
                    // Recargar la página o las listas
                    location.reload();
                });
            } else {
                Swal.fire({
